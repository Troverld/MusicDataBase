...
case class ApproveBandAuthRequestMessagePlanner(
  ...
  private case class BandAuthRequest(
    userId: String,
    bandId: String,
    status: String
  )
  ...
  private implicit val bandAuthRequestDecoder: Decoder[BandAuthRequest] =
    Decoder.forProduct3("user_id", "band_id", "status")(BandAuthRequest.apply)
  ...
  override def plan(using planContext: PlanContext): IO[(Boolean, String)] = {
    val logic: IO[String] = for {
      ...
      // 步骤 2: 从数据库检索并验证请求记录
      requestRecord <- getBandAuthRequest(requestID)
  ...
  /** 步骤2.1的实现：获取认证请求 */
  private def getBandAuthRequest(requestID: String)(using PlanContext): IO[BandAuthRequest] = {
    val sql =
      s"""
         SELECT user_id, band_id, status FROM ${schemaName}.band_auth_request_table
         WHERE request_id = ?
       """
    logInfo(s"查询 BandAuthRequestTable 获取 requestID 为 ${requestID} 的申请记录") >>
      readDBRows(sql, List(SqlParameter("String", requestID))).flatMap {
        case row :: Nil =>
          IO.fromEither(row.as[BandAuthRequest])
  ...
  /** 步骤2.2的实现：验证请求状态 */
  private def verifyRequestStatus(request: BandAuthRequest, expectedStatus: RequestStatus)(using PlanContext): IO[Unit] = {
  ...
  /** 步骤3.1的实现：批准请求 */
  private def approveRequest(request: BandAuthRequest)(using PlanContext): IO[String] = {
    for {
      _ <- logInfo(s"将请求 ${requestID} 的状态更新为 Approved")
      _ <- updateRequestStatus(RequestStatus.Approved)

      _ <- logInfo(s"调用 AddBandManager，为乐队 ${request.bandId} 绑定管理员 ${request.userId}")
      _ <- AddBandManager(adminID, adminToken, request.userId, request.bandId).send.flatMap {
        case (true, _) => IO.unit
        case (false, msg) => IO.raiseError[Unit](new Exception(s"添加乐队管理员失败: $msg"))
      }
    } yield "请求已批准"
  }
  ...
  private def updateRequestStatus(status: RequestStatus)(using PlanContext): IO[Unit] = {
    val sql =
      s"""
         UPDATE ${schemaName}.band_auth_request_table
         SET status = ?
         WHERE request_id = ?
       """
    writeDB(
