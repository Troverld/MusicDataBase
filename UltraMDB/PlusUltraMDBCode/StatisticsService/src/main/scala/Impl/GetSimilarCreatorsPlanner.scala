...
case class GetSimilarCreatorsPlanner(
  ...
  override def plan(using planContext: PlanContext): IO[(Option[List[CreatorID_Type]], String)] = {
    val logic: IO[List[CreatorID_Type]] = for {
      _ <- logInfo(s"开始查找与创作者 ${creatorID.id} (${creatorID.creatorType}) 相似的创作者，限制数量: ${limit}")
      _ <- validateUser()
      _ <- validateLimit()
      _ <- logInfo("验证通过，正在调用 GetSimilarCreatorsUtils 执行查找逻辑")
      similarCreators <- GetSimilarCreatorsUtils.findSimilarCreators(userID, userToken, creatorID, limit)
    } yield similarCreators

    logic.map { creators =>
      (Some(creators), "相似创作者查找成功")
    }.handleErrorWith { error =>
      logError(s"查找创作者 ${creatorID.id} 的相似创作者失败", error) >>
